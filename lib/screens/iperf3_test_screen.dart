...
import 'user_home_screen.dart'; // Import UserHomeScreen
import 'indoor_wifi_test.dart'; // Correct the import path

class Iperf3TestScreen extends StatefulWidget {
  final WifiTestResult testResult;

  Iperf3TestScreen({required this.testResult});
...
class _Iperf3TestScreenState extends State<Iperf3TestScreen> {
  ...
  Future<void> _runIperf3TCP() async {
    ...
    try {
      ...
      setState(() {
        ...
        widget.testResult.throughput = throughput;
        widget.testResult.transfer = transfer;
      });

      await _saveTestResultToDatabase(widget.testResult);
  ...
  Future<void> _runIperf3UDP() async {
    ...
    try {
      ...
      setState(() {
        ...
        widget.testResult.throughput = throughput;
        widget.testResult.transfer = transfer;
        widget.testResult.jitter = jitter;
      });
  ...
  Future<void> _saveTestResultToDatabase(WifiTestResult result) async {
    DatabaseHelper dbHelper = DatabaseHelper();
    await dbHelper.addWifiTestResult(result);
  }
                    ...
                    child: ElevatedButton(
                      onPressed: () {
                        Navigator.pushReplacement(
                          context,
                          MaterialPageRoute(
                            builder: (context) => WiFiTestForm(projectId: widget.testResult.projectId),
                          ),
                        );
                      },
                      style: ElevatedButton.styleFrom(
